...
const flexibleUpload = (req: Request, res: Response, next: any) => {
  upload.fields([
...
// ─────────────────────────────────────────────────────────────────────────────
async function describeImageWithGroq(
  imageBuffer: Buffer,
  mimetype: string,
): Promise<string> {
  try {
...
async function extractKeyFrames(videoPath: string, count: number = 6): Promise<Buffer[]> {
...
async function analyzeVideoWithGroq(videoPath: string): Promise<string> {
  try {
...
// ─────────────────────────────────────────────────────────────────────────────
export async function analyzeMemoryWithGroq(
  title: string,
  content: string,
  excerpt: string,
  tags: string[],
...
async function submitToGenLayer(params: {
  ...
  mediaType:     string;
  title:         string;
  footprint:     Awaited<ReturnType<typeof analyzeMemoryWithGroq>>;
}): Promise<string> {
...
const uploadMiddleware = (fieldName: string = 'media') => upload.single(fieldName);
...
function buildSemanticRelatedQuery(story: any) {
...
async function fetchSemanticFallback(story: any, limit: number) {
...
// ─────────────────────────────────────────────────────────────────────────────
app.post('/api/analyze-memory', flexibleUpload, async (req: Request, res: Response) => {
    try {
    ...
    const semanticFootprint = await analyzeMemoryWithGroq(
      title,
      content || '',
...
// ─────────────────────────────────────────────────────────────────────────────
app.post('/api/stories', flexibleUpload, async (req: Request, res: Response) => {
  // upload.single('media')(req, res,)});
  // async (req: Request, res: Response) => {
    try {
      const {
        title, excerpt, content, author, tag, tags,
        semanticFootprint: rawFootprint,
        genLayerTxHash,
      } = req.body;
...
if (req.file) {
  ...
  mediaUrl = await uploadToCloudinary(fileBuffer, req.file.mimetype);
      ...
      let footprint: Awaited<ReturnType<typeof analyzeMemoryWithGroq>> | null = null;

      if (rawFootprint) {
        try {
          const parsed =
            typeof rawFootprint === 'string' ? JSON.parse(rawFootprint) : rawFootprint;
      ...
      visualDescription = await describeImageWithGroq(mediaBuffer, req.file!.mimetype);
        ...
        footprint = await analyzeMemoryWithGroq(
          title,
          content,
          excerpt || content.substring(0, 100),
          safeTags,
      ...
      if (CONTRACT) {
        submitToGenLayer({
          ...
          .catch(err =>
            console.error('❌ GenLayer submission failed:', err.message, err.stack),  // was warn, now error+stack
          );
      }
...
// ─────────────────────────────────────────────────────────────────────────────
app.patch('/api/stories/:id/onchain', async (req: Request, res: Response) => {
  try {
    const { genLayerTxHash } = req.body;
    const updated = await StoryModel.findByIdAndUpdate(
      req.params.id,
      { genLayerTxHash, onChainConfirmed: true },
      { new: true },
    );
...
// ─────────────────────────────────────────────────────────────────────────────
app.get('/api/stories/:id/related', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const relatedOnChain: Array<{ mongo_id: string }> = JSON.parse(rawResult as string);
...
app.get('/api/stories', async (req: Request, res: Response) => {
  try {
    const query: any = {};
    if (req.query.tag && req.query.tag !== 'All') query.tag = req.query.tag;
    if (req.query.search) {
      query.$or = [
...
app.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const updated = await StoryModel.findByIdAndUpdate(
        req.params.id,
        { tag, title, excerpt, content, author, image: imageUrl, tags: safeTags },
        { new: true },
      );
      res.json(norm(updated!));
    } catch (err: any) {
...
app.delete('/api/stories/:id', async (req: Request, res: Response) => {
  try {
    const story = await StoryModel.findById(req.params.id);
    if (!story) { res.status(404).json({ error: 'Not found' }); return; }
    if (story.image) await deleteFromCloudinary(story.image);
